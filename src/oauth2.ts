...
const createRoughOAuth2Provider = (
  ...
  const { baseUrl, clientId, clientSecret, redirectUri } = options

  const authorizationEndpoint = `${baseUrl}/api/v1/oauth2/authorize`
  const tokenEndpoint = `${baseUrl}/api/v1/oauth2/token`
  const tokenRevocationEndpoint = `${baseUrl}/api/v1/oauth2/token/revoke`
  ...
  const roughOAuth2Provider: RoughOAuth2Provider = {
    ...
    validateAuthorizationCode: (code, codeVerifier) => {
      return errorBoundary(async () => {
        ...
        const request = createOAuth2Request(tokenEndpoint, body, baseUrl)
